...
import BlockChart, { INACTIVE_OUT, Data, Status } from '../BlockChart'

export class Rect {
  public parent: BlockChart
  public value: any
  ...
  public inactiveOut = 0

  // 컬러 변경 이펙트
  public prevStatus?: Status

  constructor(parent: any, data: Data, x: number, y: number, w: number, h: number) {
    this.parent = parent
  ...
  private fittingString(ctx: CanvasRenderingContext2D, str: string, maxWidth: number) {
    if (!str || !Number(maxWidth) || !ctx) return ''
  ...
  public render = () => {
    ...
    const isHover = false //this.parent.hovering === this
