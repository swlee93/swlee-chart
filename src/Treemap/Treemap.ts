class Treemap {
  ...
  public addData = (data: any, opts: any) => {
    opts = opts || {}
  ...
  public addItem = (data: any, keys: any, value: any) => {
    value = value || 1

    if (keys) {
      // Find element in hierarchy
      var currItem: any = this

      for (var j = 0; j < keys.length; j++) {
        var k = keys[j]
        var i = currItem.items.findIndex(function (el: any) {
          return el.data == data[k]
        })
      ...
      var i: any = this.items.findIndex(function (el: any) {
        return el.data == data
      })
      if (i >= 0) {
        // the element is already in this Treemap, so just increase value
  ...
  public addTreemap = (data: any, value: any) => {
  ...
  public sumUpValues = (valueKey?: string) => {
  ...
  public calculate = (valueKey?: string) => {
  ...
  public draw = (drawItemFunction: any) => {
    if (!drawItemFunction) {
